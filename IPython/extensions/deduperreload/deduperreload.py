...
class DeduperReloader(DeduperReloaderPatchingMixin):
    ...
    def update_sources(self) -> None:
        ...
        for new_modname in sys.modules.keys() - self.source_by_modname.keys():
            ...
            if (
                not (fname := get_module_file_name(new_module))
    ...
    def check_dependents_inner(
        ...
        for func_name in cur.defs_to_reload:
            ...
            ans.extend(self.gen_dependents(node))
        for class_name in cur.new_nested_classes:
            ans.extend(self.check_dependents_inner(prefixes + [class_name]))
        return ans
    ...
    def gen_dependents(self, qualname: tuple[str, ...]) -> list[DependencyNode]:
        ...
        for elt in self.dependency_graph[qualname]:
            ans.extend(self.gen_dependents(elt.qualified_name))
            ans.append(elt)
        return ans
    ...
    def _patch_namespace_inner(
        ...
        Then, replace the old function's attributes with the new function's attributes.
        """
        prefixes = prefixes or []
        cur = self._to_autoreload.traverse_prefixes(prefixes)
        ...
        for class_name in cur.children.keys():
            if not self.patch_namespace(ns, prefixes + [class_name]):
                return False
        cur.children.clear()
        return True
